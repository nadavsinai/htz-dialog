...
export default function addEventListners(dialogState) {
  ...
  const showBtns =
    [...document.querySelectorAll(`[data-htz-dialog-show="${dialogState.wrapperId}"]`)];

  const outOfWrapperHideBtns =
    [...document.querySelectorAll(`[data-htz-dialog-hide="${dialogState.wrapperId}"]`)];
